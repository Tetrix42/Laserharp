...
class music:
	...
	def __init__(self, beams, mods = 0, lowestpitch = 60, useFullSteps = True, instrument = 19):	## sets the number of beams. and builds the status array.
		self.beams = beams;
		self.active = {}
		...
		for m in range(self.modifier):
			for b in range(self.beams):
				...
				self.active[pit]= 0;
				self.midiout.send_message([0xC0+b,self.intstrument])
	...
	def pitch(self, beam, modifier = 0, full = True):
		...
		#self.midiout.send_message([0xC0+beam,self.intstrument])
