...
def distanceOfLineAndPoint(coord11,coord12,coord21,coord22,cor1,cor2):
    ...
    if compare <= 0:
        if disBeginningOfLineToPoint < disEndOfLineToPoint: # These two conditions ensure that the point is nearer to the Beginning
            ratio=0 # If line is too short such that point is outwards of line on the other side
                    # also just connect to the front point of the line
    if ratio > 1:
...
if ak2 == 'q':
    sys.exit()
while(1):
    cor = readInputUntilRecognition()
    print "Cor-Values: "+str(cor)
    if cor:
        drawDistanceToNearestLine(allCoords,cor)
